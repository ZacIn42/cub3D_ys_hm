...
/*   Updated: 2024/08/03 20:17:00 by hmiyazak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_wall(t_cub *cub, t_field *field)
{
	...
	theta_start = M_PI / 6;
	theta_end = M_PI * 5 / 6;
	theta_step = (theta_end - theta_start) / WIN_WIDTH;
	x = 0;
	while (x < WIN_WIDTH)
	{
		spot = cast_ray_alloc(&field->user.pos, theta_start + theta_step * x, field->map);
